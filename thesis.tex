  ...
  \subsection{Microphysics}
    ...
    \subsection{Testing Simulation Equation Tolerances}
      ...
      \begin{figure}
        ...
        \caption{Relative error over time of the \ce{^4H3} (in black), \ce{^{12}C} (in purple), and \ce{^{16}O} (in blue) mass fractions run in simulations with a tolerance of $10^{-9}$ (solid line), $10^{-6}$ (dashed line), and $10^{-3}$ (dotted line) with respect to the true values calculated by a simulation with a tolerance of $10^{-12}$. The lower the value, the closer the result is to the true value. % Should I add the bit about why 10^-9 is the best here or leave that outside of the figure caption?
        %The relative error of the simulation with a tolerance of $10^{-12}$ is consistently below $10^{-3}$ and drops as low as $10^{-8} at times without taking much longer to run than the simulations with tolerances of $10^{-6}$ and $10^{-3}$.
          }
    ...
    \subsection{Using Mass Fraction Evolution to Justify Motivation}
      ...
      \begin{figure}
        ...
        \caption{Mass fraction, $X$, evolution over time for the 11 elements (differentiated by color) in the {\tt subch} network for three different {\tt burn\_cell} simulations: one without \ce{^{14}C} and \ce{^{14}O} (solid lines), one with both \ce{^{14}C} and \ce{^{14}O} (dashed line), and with \ce{^{14}O} but no \ce{^{14}C} (dotted line). % To add or not to add the short analysis
        % If incorporating \ce{^{14}C} and \ce{^{14}O} has no effect on the $X$ evolution there will be only 11 solid colored lines. Seeing a combination of solid, dashed, and dotted lines at different orders of magnitudes for many elements implies that the incorporation of \ce{^{14}C} and \ce{^{14}O} is important. 
          }
...
\bibliography{../../Labs-scripts/Lab1/references}
